  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (userType === "admin" && currentUserId) {
        path = `Login/${currentUserId}/Perfil`; // Path for admin users
      } else if (userType === "vendedor" && adminParentId) {
        path = `Login/${adminParentId}/Perfil`; // Path for vendor users
      } else {
        console.error("User type is not set correctly or adminPai is missing");
        return; // Exit if necessary values aren't present
      }
