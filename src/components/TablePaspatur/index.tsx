...
export default function TablePaspatur({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Verifica se o código está rodando no lado do cliente onde o localStorage está disponível
      if (typeof window !== "undefined") {
      ...
      // Determina o caminho para a coleção baseado no tipo de usuário e adminPai
      let path;
      if (userType === "admin" && currentUserId) {
        path = `Login/${currentUserId}/Paspatur`; // Caminho para admin
      } else if (userType === "vendedor" && adminParentId) {
        path = `Login/${adminParentId}/Paspatur`; // Caminho para vendedor
      } else {
  ...
  const handleDeleteItem = async (itemId: string) => {
    try {
      await deleteDoc(doc(db, `Login/lB2pGqkarGyq98VhMGM6/Paspatur`, itemId));
      console.log("Deleting item: ", itemId);

      const updatedData = filteredData.filter((item) => item.id !== itemId);
      setFilteredData(updatedData);

      toast.success("Paspatur excluído com sucesso!", {
        style: {
          fontSize: "12px",
          fontWeight: 600,
        },
