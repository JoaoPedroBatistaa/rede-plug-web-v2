...
export default function NewPost() {
  ...
  async function sendDischargeMessage(data: {
    ...
    try {
      ...
      // Enviar mensagem simplificada para o segundo número adicional
      let simplifiedB1MessageBody = `*Nova Descarga Realizada*\n\n*Data*: ${formattedDate}\n*Hora*: ${data.time}\n*Posto*: ${data.postName}\n*Responsável*: ${data.makerName}\n\n*Motorista*: ${data.driverName}\n*Tanque*: ${data.selectedTankDescription}\n*Produto*: ${data.product}\n${data.totalLiters} litros\n*Observações*: ${data.observations}`;

      // Adicionar dados adicionais se presentes
      if (data.weight) {
        simplifiedB1MessageBody += `\n\n*Peso*: ${data.weight}`;
      }
      if (data.temperature) {
        ...
        if (weightTemperatureImage) {
          simplifiedB1MessageBody += `\n*Peso e Temperatura:* ${weightTemperatureImage.url}`;
        }
      }
      if (data.productQuality) {
        simplifiedB1MessageBody += `\n*Qualidade do Produto*: ${data.productQuality}`;
        const productQualityImage = imagesDescription.find(
          (img: { description: string }) =>
            img.description === "Imagem da Qualidade do Produto"
        );
        if (productQualityImage) {
          simplifiedB1MessageBody += `\n*Qualidade do Produto:* ${productQualityImage.url}`;
        }
      }

      const simplifiedB1MessageResponse = await fetch("/api/send-message", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          managerContact: "5511979525519",
          messageBody: simplifiedB1MessageBody,
        }),
      });

      if (!simplifiedB1MessageResponse.ok) {
        const errorMessage = await simplifiedB1MessageResponse.json();
        console.error(
          "Falha ao enviar mensagem simplificada via WhatsApp: ",
          errorMessage
        );
      ...
      // Enviar terceira mensagem simplificada para o terceiro número adicional
      let simplifiedB2MessageBody = `*Nova Descarga Realizada*\n\n*Data*: ${formattedDate}\n*Hora*: ${data.time}\n*Posto*: ${data.postName}\n*Responsável*: ${data.makerName}\n\n*Motorista*: ${data.driverName}\n*Tanque*: ${data.selectedTankDescription}\n*Produto*: ${data.product}\n${data.totalLiters} litros\n*Observações*: ${data.observations}`;

      if (data.hydration?.value) {
        simplifiedB2MessageBody += `\n\n*Hidratação*: ${data.hydration.value}\n`;
        const hydrationImage = imagesDescription.find(
          (img) => img.description === "Imagem da Hidratação"
        );
        if (hydrationImage) {
          simplifiedB2MessageBody += `*Hidratação:* ${hydrationImage.url}\n\n`;
        }
      }

      const simplifiedB2MessageResponse = await fetch("/api/send-message", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          managerContact: "5511983610285",
          messageBody: simplifiedB2MessageBody,
        }),
      });

      if (!simplifiedB2MessageResponse.ok) {
        const errorMessage = await simplifiedB2MessageResponse.json();
        console.error(
          "Falha ao enviar mensagem simplificada via WhatsApp: ",
          errorMessage
        );
