...
export default function NewPost() {
  ...
  const saveMeasurement = async () => {
    ...
    if (!querySnapshot.empty) {
      toast.error(
        "A tarefa placas de sinalização já foi feita para esse turno hoje!"
      );
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // @ts-ignore
      router.push(
        `/supervisors/supplier-identification?post=${encodeURIComponent(
          // @ts-ignore
          postName
        )}&shift=${shift}`
      );
      ...
      <div className={styles.Container}>
        <div className={styles.BudgetContainer}>
          <div className={styles.BudgetHead}>
            <p className={styles.BudgetTitle}>Placas de Sinalização</p>
            {!docId && (
          ...
          <p className={styles.Notes}>
            Informe abaixo as informações das placas de sinalização
          </p>
