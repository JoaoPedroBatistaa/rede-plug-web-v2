...
export default function NewPost() {
  ...
  const saveMeasurement = async () => {
    ...
    const querySnapshot = await getDocs(q);
    if (!querySnapshot.empty) {
      toast.error("A tarefa extintores já foi feita para esse turno hoje!");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // @ts-ignore
      router.push(
        `/supervisors/gauges?post=${encodeURIComponent(
          // @ts-ignore
          postName
        )}&shift=${shift}`
      );
      ...
      <div className={styles.Container}>
        <div className={styles.BudgetContainer}>
          ...
          <p className={styles.Notes}>
            Informe abaixo as informações dos extintores
          </p>
