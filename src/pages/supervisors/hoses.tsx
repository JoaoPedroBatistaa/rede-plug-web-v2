...
export default function NewPost() {
  ...
  const saveMeasurement = async () => {
    ...
    const querySnapshot = await getDocs(q);
    if (!querySnapshot.empty) {
      toast.error("A tarefa mangueiras já foi feita para esse turno hoje!");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      // @ts-ignore
      router.push(
        `/supervisors/bomb-seal?post=${encodeURIComponent(
          // @ts-ignore
          postName
        )}&shift=${shift}`
      );
      ...
      <div className={styles.Container}>
        <div className={styles.BudgetContainer}>
          <div className={styles.BudgetHead}>
            <p className={styles.BudgetTitle}>Mangueiras</p>
            {!docId && (
          ...
          <p className={styles.Notes}>
            Informe abaixo as informações das mangueiras
          </p>
